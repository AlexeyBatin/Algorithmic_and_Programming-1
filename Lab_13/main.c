...
void processing(int a[N][N],int n,int *counter,int *elements) // Обробка масиву 
{
  ...
  *counter = 0;
  *elements = 0;
  for (i=0; i<n; i++) // Розрахунки в 3-му секторі
    for (j=0; j<n; j++)
      if (i+j>n-1 && i-j<=0 && (a[i][j])%2==0 ) *counter+=1;
  if(n%2) {  // Розрахунки в 9-му секторі
    for (i=n/2; i<n; i++)
      for (j=0; j<n; j++) {a[i][j]=0;*elements+=1;} // заміна елементів на нулі і лічильник
  }
  else {
    for (i=(n+1)/2; i<n; i++)
      for (j=0; j<n; j++)  {a[i][j]=0;*elements+=1;} // заміна елементів на нулі і лічильник
...
int main() {
  int a[N][N];  // вказівник на вказівник на рядок
  ...
  while (ind) {
    ...
    do {
      ...
      if(n>0 && n<N)  ind = 0; }
    while (ind);
    // Виділення пам'яті під вказівник на рядки
    ...
    scanf("%s",&d);
    if(d[0] =='y' ) { ind = 1; system("cls");} // звільняє місце в пам'яті , очищує консоль
    else ind = 0;
  }
  system("pause");
  return 0;
