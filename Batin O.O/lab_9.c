...
int main () {
  ...
  printf("----------------------------------------------------------\n");
  ind = 1;
  while (ind) { // Введення значення х
    printf("\tВкажіть значення х [-10;10]: ");
    scanf_s("%lf",&x);
    if(x<-10 || x>10) printf("\tВказали значення x не з проміжку\n\a"); // Помилка у введені даних
    else ind = 0; 
  }
  ind=1;
  while (ind) { // Введення значення епсілон
  ...
  ind=1;
  while (ind){ // введення значення ліміту
  ...
  do {
    i++;  
    an*=-x2/(i*(i*2+1)); // Обчислення значення ряду
    s+=an; 
  ...
  f=sin(x); // Обчислення точного значення функціх sin(x)
