...
Лабораторна робота №7 Проектування алгоритмів і програм циклічної структури (оператори циклів з передумовою і післяумовою)*/
int main () {
  ...
  scanf("%lf %lf",&a,&b); // значення параметрів а і б
  printf("%s\n",r1); 
  printf("%s%s%s|\n",r2,r2,r2); // Верхній рядок таблиці
  printf("%s\n",r1);
  x=x1;
  i=0;
  while (x<=x2+eps){
    i++;ind=1;
    printf("| %6.2lf",x); // Виведення таблиці
    if (x+a<0 || fabsl(sin(x-b))<eps) printf(" |  не визн ");
    else { f=(sqrt(x+a)/sin(x-b))+cos(fabs(x)); // обчислення функції
